...
DESIRED_REPOSITORY = "https://github.com/GeographicaGS/urbo-watcher.git"
...
pipeline{
  ...
  stages {
    ...
    stage ('Building') {
      ...
      steps {
        sh "docker build --pull=true -f Dockerfile.new -t geographica/urbo-watcher:${git_commit} ."
      }
    }
      ...
      steps{
        script {
          ...
          withCredentials([[$class: 'UsernamePasswordMultiBinding',credentialsId: 'dockerhub',usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
            ...
            sh "docker tag geographica/urbo-watcher:${ git_commit } geographica/urbo-watcher:${ env.DEPLOY_TO }"
            sh "docker tag geographica/urbo-watcher:${ git_commit } geographica/urbo-watcher:${ env.DEPLOY_TO }-${ build_name }"
            sh "docker push geographica/urbo-watcher:${ env.DEPLOY_TO }"
            sh "docker push geographica/urbo-watcher:${ env.DEPLOY_TO }-${ build_name }"
          }
        }
      }
    }
